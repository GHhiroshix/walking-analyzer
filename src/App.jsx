...
- 補助具（杖・歩行器・シルバーカーなど）の使用そのものは減点対象にしないでください。ただし、点数を無理に底上げすることも禁止です。実際の歩行の安定性・バランス・転倒リスクを見たまま正直に採点してください。
- ふらつきや不安定さが見られる場合は、補助具の有無にかかわらず、その危険度に応じて正直に低い点数をつけてください。
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      try {
        ...
        // スコアはAIの採点結果をそのまま使用します（安全性のため、強制的な底上げは行いません）
