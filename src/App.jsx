...
export default function WalkingVideoAnalyzer() {
  ...
  if (phase==="print"&&result) {
    const sc2=result.score>=75?"#2d6a4f":result.score>=50?"#b5451b":"#c1121f";
    const handlePrint = () => {
      let html=`<!DOCTYPE html><html lang="ja"><head><meta charset="utf-8"><title>歩行解析レポート - ${patientName}</title><link href="https://fonts.googleapis.com/css2?family=Kosugi+Maru&display=swap" rel="stylesheet"><style>body{font-family:'Kosugi Maru',sans-serif;color:#2c2c2c;padding:20px;max-width:700px;margin:0 auto;background:#f5f0eb;}h2{margin:0;}table{width:100%;border-collapse:collapse;}td{padding:8px;border-bottom:1px solid #f0ebe4;}@media print{@page{margin:15mm;}}</style></head><body>`;
      html+=`<div style="border-bottom:3px solid #2d6a4f;padding-bottom:12px;margin-bottom:20px;display:flex;justify-content:space-between;"><div><div style="font-size:10px;color:#888;letter-spacing:2px;">VIDEO GAIT ANALYSIS REPORT</div><h2 style="font-size:24px;">${patientName} 様</h2></div><div style="text-align:right;font-size:11px;color:#888;"><div>${formatDate(patientHistory[0]?.date)}</div><div>${patientHistory.length}回目の測定</div></div></div>`;
      html+=`<div style="display:flex;align-items:center;gap:20px;margin-bottom:20px;padding:20px;background:#fff;border-radius:14px;border:2px solid ${sc};"><div style="text-align:center;min-width:80px;"><div style="font-size:56px;font-weight:900;color:${sc};font-family:monospace;">${result.score}</div><div style="font-size:10px;color:#888;">GAIT SCORE</div></div><div style="flex:1;border-left:3px solid ${sc};padding-left:16px;"><div style="font-size:16px;font-weight:700;margin-bottom:6px;">${result.summary}</div>${result.progress?`<div style="font-size:12px;color:#444;padding:8px;background:#eaf4fb;border-left:3px solid #3b82f6;">${result.progress}</div>`:""}</div></div>`;
      if(result.gait){html+=`<div style="margin-bottom:16px;background:#fff;border-radius:12px;padding:16px;border:1px solid #d9cfc4;"><div style="font-size:11px;color:#888;letter-spacing:2px;margin-bottom:12px;font-weight:700;">📊 歩行指標</div><table><tbody>${[["歩行リズム",result.gait.cadence],["歩幅",result.gait.stride],["体幹・姿勢",result.gait.posture],["腕振り",result.gait.armSwing],["足のクリアランス",result.gait.footClearance]].map(([l,v])=>`<tr><td style="color:#666;width:130px;font-weight:600;">${l}</td><td>${v}</td></tr>`).join("")}</tbody></table></div>`;}
      if(result.issues&&result.issues.length>0){html+=`<div style="margin-bottom:16px;"><div style="font-size:11px;color:#888;letter-spacing:2px;margin-bottom:10px;font-weight:700;">⚠️ 課題</div>${result.issues.map(i=>`<div style="margin-bottom:8px;padding:10px 14px;background:#fff;border-left:4px solid ${i.severity==="high"?"#dc2626":i.severity==="medium"?"#d97706":"#16a34a"};border-radius:0 10px 10px 0;"><div style="font-weight:700;margin-bottom:4px;">${i.title}</div><div style="font-size:12px;color:#555;line-height:1.6;">${i.detail}</div></div>`).join("")}</div>`;}
      if(result.exercises&&result.exercises.length>0){html+=`<div style="margin-bottom:16px;"><div style="font-size:11px;color:#888;letter-spacing:2px;margin-bottom:10px;font-weight:700;">🏃 体操メニュー</div>${result.exercises.map(ex=>`<div style="margin-bottom:10px;padding:12px 14px;background:#f0faf4;border-radius:10px;border:1px solid #a7f3d0;"><div style="font-weight:700;color:#065f46;margin-bottom:6px;">${ex.name} <span style="font-size:11px;color:#888;font-weight:400;">(${ex.target} / ${ex.duration})</span></div>${ex.steps.map((s,j)=>`<div style="font-size:12px;color:#444;margin-left:12px;margin-bottom:3px;">${j+1}. ${s}</div>`).join("")}<div style="font-size:12px;color:#065f46;font-weight:600;margin-top:6px;">✓ 効果：${ex.effect}</div></div>`).join("")}</div>`;}
      if(result.lifestyle&&result.lifestyle.length>0){html+=`<div style="margin-bottom:16px;"><div style="font-size:11px;color:#888;letter-spacing:2px;margin-bottom:10px;font-weight:700;">💡 生活アドバイス</div>${result.lifestyle.map((t,i)=>`<div style="font-size:13px;color:#444;padding:8px 14px;background:#fffbeb;border-radius:8px;border:1px solid #fde68a;margin-bottom:8px;line-height:1.6;">${i+1}. ${t}</div>`).join("")}</div>`;}
      html+=`<div style="margin-top:20px;padding-top:12px;border-top:1px solid #d9cfc4;font-size:10px;color:#aaa;text-align:center;">本レポートはAI歩行解析の参考情報です。医療診断の代替ではありません。</div></body></html>`;
