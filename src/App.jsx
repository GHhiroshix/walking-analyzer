    ...
    return (
      <div style={wrap}><GlassOrbs/><div style={maxW}><div style={{paddingTop:32}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:20}}>
          <button onClick={()=>{
            const sc=result.score>=75?"#16a34a":result.score>=50?"#d97706":"#dc2626";
            let html=`<!DOCTYPE html><html lang="ja"><head><meta charset="utf-8"><title>歩行解析レポート - ${patientName}</title><link href="https://fonts.googleapis.com/css2?family=Kosugi+Maru&display=swap" rel="stylesheet"><style>body{font-family:'Kosugi Maru',sans-serif;color:#111;padding:20px;max-width:700px;margin:0 auto;}h2{margin:0;}table{width:100%;border-collapse:collapse;}td{padding:6px 8px;border-bottom:1px solid #f0f0f0;}@media print{@page{margin:15mm;}}</style></head><body>`;
            html+=`<div style="border-bottom:2px solid #39e0b0;padding-bottom:12px;margin-bottom:20px;display:flex;justify-content:space-between;"><div><div style="font-size:10px;color:#666;letter-spacing:2px;">VIDEO GAIT ANALYSIS REPORT</div><h2>${patientName} 様</h2></div><div style="text-align:right;font-size:11px;color:#666;"><div>${formatDate(new Date().toISOString())}</div><div>${patientHistory.length}回目の測定</div></div></div>`;
            html+=`<div style="display:flex;align-items:center;gap:20px;margin-bottom:20px;padding:16px;background:#f8f8f8;border-radius:12px;"><div style="text-align:center;"><div style="font-size:48px;font-weight:900;color:${sc};font-family:monospace;">${result.score}</div><div style="font-size:10px;color:#666;">GAIT SCORE</div></div><div><div style="font-size:16px;font-weight:700;margin-bottom:4px;">${result.summary}</div>${result.progress?`<div style="font-size:12px;color:#444;padding:8px;background:#e8f4fd;border-left:3px solid #3b82f6;">${result.progress}</div>`:""}</div></div>`;
            if(result.gait){html+=`<div style="margin-bottom:16px;"><div style="font-size:11px;color:#666;letter-spacing:2px;border-bottom:1px solid #eee;padding-bottom:6px;margin-bottom:10px;">歩行指標</div><table><tbody>${[["歩行リズム",result.gait.cadence],["歩幅",result.gait.stride],["体幹・姿勢",result.gait.posture],["腕振り",result.gait.armSwing],["足のクリアランス",result.gait.footClearance]].map(([l,v])=>`<tr><td style="color:#666;width:120px;">${l}</td><td style="font-weight:600;">${v}</td></tr>`).join("")}</tbody></table></div>`;}
            if(result.issues&&result.issues.length>0){html+=`<div style="margin-bottom:16px;"><div style="font-size:11px;color:#666;letter-spacing:2px;border-bottom:1px solid #eee;padding-bottom:6px;margin-bottom:10px;">課題</div>${result.issues.map(i=>`<div style="margin-bottom:8px;padding:8px 12px;background:#fafafa;border-left:3px solid ${i.severity==="high"?"#dc2626":i.severity==="medium"?"#d97706":"#16a34a"};"><div style="font-weight:700;margin-bottom:2px;">${i.title}</div><div style="font-size:12px;color:#444;">${i.detail}</div></div>`).join("")}</div>`;}
