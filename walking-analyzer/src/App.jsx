...
function scoreDiff(cur, prev) { const d = cur - prev; if (d > 0) return { label: `+${d}`, color: C.accent }; if (d < 0) return { label: `${d}`, color: C.red }; return { label: "±0", color: C.muted }; }const buildPrompt = (frameCount, history) => {
...
{"score":数値,"summary":"総合評価","progress":"前回比コメント","aids":{"detected":[],"usage":null,"recommendation":null},"gait":{"cadence":"","stride":"","posture":"","armSwing":"","footClearance":""},"issues":[{"title":"","detail":"","severity":"high|medium|low"}],"exercises":[{"name":"","target":"","duration":"","steps":[],"effect":"","isNew":true}],"lifestyle":[]}`;
};
  ...
  if (phase==="userSelect") {
    ...
    return (
      <div style={wrap}><div style={maxW}>
        ...
        {patients.length>0&&(<div style={{marginBottom:20}}><div style={{fontSize:11,color:C.muted,letterSpacing:2,marginBottom:10}}>登録済み利用者</div><div style={{display:"flex",flexDirection:"column",gap:8}}>{patients.map(p=>{ const hist=p.history||[], last=hist[0]; return (<div key={p.id} onClick={()=>selectExisting(p)} style={{display:"flex",alignItems:"center",gap:12,background:C.surface,border:`1.5px solid ${C.border}`,borderRadius:12,padding:"14px 16px",cursor:"pointer",transition:"all 0.15s"}}><div style={{width:40,height:40,borderRadius:"50%",background:C.panel,border:`2px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>👤</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,fontSize:15,color:C.text}}>{p.name}</div><div style={{fontSize:11,color:C.muted,marginTop:2}}>{hist.length>0?`${hist.length}回測定済み ／ 最終: ${formatDate(last.date)} ／ スコア: ${last.score}`:"測定歴なし"}</div></div><div style={{color:C.muted,fontSize:16}}>›</div></div>); })}</div></div>)}
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px"}}><div style={{fontSize:11,color:C.muted,letterSpacing:2,marginBottom:12}}>新規登録</div><div style={{display:"flex",gap:8}}><input value={nameInput} onChange={e=>setNameInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addNew()} placeholder="お名前またはID（例：田中さん）" style={{flex:1,background:C.panel,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:13,fontFamily:"'Noto Sans JP',sans-serif",outline:"none"}}/><button onClick={addNew} disabled={!nameInput.trim()} style={{padding:"10px 16px",background:nameInput.trim()?C.accent:C.border,border:"none",borderRadius:8,color:nameInput.trim()?C.bg:C.muted,fontSize:13,fontWeight:700,cursor:nameInput.trim()?"pointer":"not-allowed",fontFamily:"'Noto Sans JP',sans-serif",whiteSpace:"nowrap"}}>登録して開始</button></div></div>
      </div></div>
    );
  }
